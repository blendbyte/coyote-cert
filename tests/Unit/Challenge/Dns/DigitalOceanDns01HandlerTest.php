...
it('cleanup deletes every record deployed for the same domain (wildcard + base)', function () {
    ...
    expect($client->captured[2])->toMatchArray(['method' => 'DELETE', 'path' => '/domains/example.com/records/11']);
    expect($client->captured[3])->toMatchArray(['method' => 'DELETE', 'path' => '/domains/example.com/records/22']);
});
