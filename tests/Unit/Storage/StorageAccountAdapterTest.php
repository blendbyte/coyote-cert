...
it('exists returns false when no account key is stored', function () {
    ...
    $adapter = new StorageAccountAdapter($storage, 'letsencrypt');
...
it('generateNewKeys stores a key in storage', function () {
    $storage = new InMemoryStorage();
    $adapter = new StorageAccountAdapter($storage, 'letsencrypt', KeyType::RSA_2048);
...
it('getPublicKey() throws CryptoException when the stored key PEM is not a valid private key', function () {
    $storage = new InMemoryStorage();
    $storage->saveAccountKey('letsencrypt', 'this-is-not-a-valid-pem', KeyType::RSA_2048);
    $adapter = new StorageAccountAdapter($storage, 'letsencrypt');
...
it('savePrivateKey() delegates to storage->saveAccountKey()', function () {
    ...
    expect($storage->hasAccountKey('letsencrypt'))->toBeTrue();
    expect($storage->getAccountKey('letsencrypt'))->toBe($pem);
    expect($storage->getAccountKeyType('letsencrypt'))->toBe(KeyType::RSA_2048);
});
